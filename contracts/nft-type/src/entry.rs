...
use self::base::Base;
mod anyone_can_pay;
use self::anyone_can_pay::DisableAnyoneCanPay;
...
macro_rules! define_nft_type {
    ($($contract:ident),*) => {
        fn handle_creation(nft_type: &Script) -> Result<(), Error> {
            // parameter contracts evaluation
            [$($contract::handle_creation(nft_type)?),*];
            // base behavior
            let nft_args: Bytes = nft_type.args().unpack();
            let class_inputs_count =
                count_cells_by_type(Source::Input, &check_class_type(&nft_args));
            if class_inputs_count != 1 {
                return Err(Error::ClassCellsCountError);
            }

            let load_class =
                |source| match load_cell_data_by_type(source, &check_class_type(&nft_args)) {
            ...
            let nft_outputs_increased_count =
                (output_class.nft_count - input_class.nft_count) as usize;
        ...
        fn handle_update(nft_type: &Script) -> Result<(), Error> {
            // parameter contracts evaluation
            [$($contract::handle_update(nft_type)?),*];
            // base behavior
            let nft_data = (
        ...
        fn handle_destroying(nft_type: &Script) -> Result<(), Error> {
            // parameter contracts evaluation
            [$($contract::handle_destroying(nft_type)?),*];
            Ok(())
        }
    };
}
...
pub fn main() -> Result<(), Error> {
    ...
    define_nft_type! { Base, DisableAnyoneCanPay }
